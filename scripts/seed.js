...
async function main() {
  ...
  const gsnt = await ethers.getContractAt(
    "Token",
    config[chainId].gsnt.address
  );
  console.log(`GASton Token fetched: ${gsnt.address}\n`);
  ...
  // Send GASton tokens to investor 1
  ...
  // Send GASton tokens to investor 3
  transaction = await gsnt
    .connect(deployer)
  ...
  transaction = await gsnt.connect(deployer).approve(amm.address, amount);
  await transaction.wait();
  ...
  /////////////////////////////////////////////////////////////
  // Investor 1 Swaps: GSNT --> USD
  //

  console.log(`Investor 1 Swaps...\n`);

  // Investor approves all tokens
  transaction = await gsnt.connect(investor1).approve(amm.address, tokens(10));
  await transaction.wait();
  ...
  /////////////////////////////////////////////////////////////
  // Investor 3 Swaps: GSNT --> USD
  //

  console.log(`Investor 3 Swaps...\n`);

  // Investor approves all tokens
  transaction = await gsnt.connect(investor3).approve(amm.address, tokens(10));
  await transaction.wait();
